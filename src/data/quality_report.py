...
AGGREGATE_DETAILED_METADATA: dict[str, dict[str, str]] = {
    ...
    "INSTAL_LATE_COUNT": {
        ...
        "source_grain": "installment payment record",
        "transformation_formula": "sum(DAYS_ENTRY_PAYMENT > DAYS_INSTALMENT where both timing fields are observed)",
        "description": "Total number of timing-observed installment payments paid past the scheduled due date",
        "missing_value_meaning": "Missing prior to join fill; zero indicates no confirmed late payment. Records with an unknown due or payment date are excluded from this count and the late-rate denominator.",
        "as_of_time_rule": "DAYS_INSTALMENT <= 0 and DAYS_ENTRY_PAYMENT <= 0.",
        "leakage_note": "Installment schedules and payments executed strictly prior to application date; no target or post-application data used.",
    },
    "INSTAL_LATE_RATE": {
        ...
        "source_columns": "SK_ID_CURR | DAYS_INSTALMENT | DAYS_ENTRY_PAYMENT",
        "source_grain": "installment payment record",
        "transformation_formula": "count(DAYS_ENTRY_PAYMENT > DAYS_INSTALMENT) / count(records with both timing fields observed)",
        "description": "Share of late payments among installment records with observed due and payment dates",
        "missing_value_meaning": "Missing when customer has no observed timing denominator (no installment with both dates); 0 when observed timing exists but no payment was late. Unknown timing is not treated as on-time.",
        "as_of_time_rule": "DAYS_INSTALMENT <= 0 and DAYS_ENTRY_PAYMENT <= 0.",
        "leakage_note": "Installment schedules and payments executed strictly prior to application date; no target or post-application data used.",
    },
    "INSTAL_DELAY_DAYS_MEAN": {
        ...
        "description": "Average payment delay in days across installments with observed due and payment dates (non-late installments count as 0)",
        "missing_value_meaning": "Missing when customer has no installment with both timing fields observed.",
        "as_of_time_rule": "DAYS_INSTALMENT <= 0 and DAYS_ENTRY_PAYMENT <= 0.",
        "leakage_note": "Installment schedules and payments executed strictly prior to application date; no target or post-application data used.",
    },
    "INSTAL_DELAY_DAYS_MAX": {
        ...
        "description": "Maximum payment delay in days observed on any installment with both timing fields observed",
        "missing_value_meaning": "Missing when customer has no installment with both timing fields observed.",
        "as_of_time_rule": "DAYS_INSTALMENT <= 0 and DAYS_ENTRY_PAYMENT <= 0.",
        "leakage_note": "Installment schedules and payments executed strictly prior to application date; no target or post-application data used.",
    },
